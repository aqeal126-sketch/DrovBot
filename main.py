...
# تم تحديث الحساب والقناة مالتك هنا مباشرة غصباً عن الكود القديم
...
conn = sqlite3.connect('sovereign_store_v3.db', check_same_thread=False)
cursor = conn.cursor()
...
def get_store_keyboard(parent_id=0):
    ...
    # زر العودة داخل المتجر
    if parent_id != 0:
        cursor.execute('SELECT parent_id FROM elements WHERE id=?', (parent_id,))
        gp = cursor.fetchone()
        gp_id = gp[0] if gp else 0
        kb.append([InlineKeyboardButton(text="🔙 رجوع", callback_data=f"view_{gp_id}")])
    else:
...
async def start_cmd(message: types.Message):
    ...
    # التحقق من وجود رابط إحالة (دعوة)
    ...
    if not exists:
        ...
        conn.commit()
        # مكافأة الشخص الذي دعاه بنقاط ترحيبية (5 نقاط)
        if referred_by:
    ...
    if user_id == SUPER_ADMIN:
        welcome += "👑 مرحباً بك يا سيادة المالك المطلق، كامل الصلاحيات مفعّلة لك الآن من زر الإعدادات أدناه."
    else:
...
# --- معالجة الأزرار الثابتة حسب المخطط ---
...
async def main_referral(call: types.CallbackQuery):
    ...
    text += "شارك الرابط الخاص بك مع أصدقائك أو في المجموعات، وكل شخص يدخل للبوت عن طريقك ستحصل فوراً على **5 نقاط مجانية** في رصيدك لشراء المنتجات!\n\n"
    text += f"رابطك الخاص:\n`{ref_link}`"
...
# --- ⚙️ لوحة الإعدادات والتحكم للمالك فقط (صلاحيات كاملة) ---
...
async def super_admin_panel(call: types.CallbackQuery):
    ...
    await call.message.edit_text("⚙️ **لوحة المالك العليا والتحكم المطلق بالمتجر**:\nيمكنك إدارة الأزرار وبناء محتوى البوت الشجري بالكامل من هنا بسهولة:", reply_markup=kb)
