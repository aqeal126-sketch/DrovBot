...
# تم تغيير اسم القاعدة إلى v7 لإنشاء عمود السعر (price) الجديد بدون أخطاء
...
# جدول الأزرار والأقسام الديناميكية (تمت إضافة السعر price)
...
class SystemStates(StatesGroup):
    wait_name = State()
    wait_price = State()  # حالة جديدة لانتظار السعر
    wait_type = State()
    wait_content = State()
...
def get_store_keyboard(parent_id=0):
    cursor.execute('SELECT id, name, type, content, price FROM elements WHERE parent_id=?', (parent_id,))
    ...
    for item in items:
        ...
        if item_type == 'link':
            ...
            # دمج السعر مع اسم المنتج في الزر
...
async def start_cmd(message: types.Message):
    ...
    user_balance_disp = int(user_balance) if float(user_balance).is_integer() else user_balance

    welcome = (  
        f"أهلاً بكم في -  Drov TG   👋\n\n"  
        f"🚀 أقوى سوق لبيع وشراء حسابات تيليجرام الجاهزة والجديدة لجميع الدول حول العالم 🌐.\n\n"  
        f"-  ايديك: `{user_id}` 🆔.\n"  
        f"- 👍 رصيدك:  `{user_balance_disp}` نقطة 💵.\n\n"  
...
async def main_charge(call: types.CallbackQuery):
    ...
    balance_disp = int(balance) if float(balance).is_integer() else balance

    text = f"💰 **قسم شحن الرصيد والنقاط**\n\n"  
    text += f"💳 رصيدك الحالي: `{balance_disp}` نقطة.\n\n"  
...
async def get_daily_gift(call: types.CallbackQuery):
    ...
    if last_date == today_str:  
        ...
        text = f"💰 **قسم شحن الرصيد والنقاط**\n\n💳 رصيدك الحالي: `{balance_disp}` نقطة.\n\nتم تحديث الرصيد بنجاح!"  
        kb = InlineKeyboardMarkup(inline_keyboard=[  
            [InlineKeyboardButton(text="🎁 استلام الهدية اليومية الفورية", callback_data="get_daily_gift")],  
            [InlineKeyboardButton(text="📥 تواصل معي للشراء والشحن", url=MY_ACCOUNT_URL)],  
            [InlineKeyboardButton(text="🔙 رجوع", callback_data="back_to_main")]  
...
# --- معالجة الأزرار الديناميكية للتنقل والشراء الفعلي ---
...
async def navigate_system(call: types.CallbackQuery):
    ...
    cursor.execute('SELECT type, name, content, price FROM elements WHERE id=?', (target_id,))  
    item = cursor.fetchone()  
    if not item: return await call.answer("العنصر المطلوب غير متوفر حالياً!")  

    item_type, name, content, price = item  
    user_id = call.from_user.id
    ...
    if item_type == 'folder':  
        ...
        if user_balance < price:
            price_text = int(price) if price.is_integer() else price
            balance_text = int(user_balance) if float(user_balance).is_integer() else user_balance
            return await call.answer(f"❌ رصيدك غير كافٍ!\nسعر المنتج: {price_text} نقطة\nرصيدك الحالي: {balance_text} نقطة", show_alert=True)
            
        # خصم الرصيد وتسجيل الشراء
        ...
        await call.answer("✅ تمت عملية الشراء بنجاح!", show_alert=False)
        price_text = int(price) if price.is_integer() else price
        
        if item_type == 'text':  
            await call.message.answer(f"📦 **{name}**\n\n{content}\n\n*(تمت إضافة هذا المنتج لمشترياتك وتم خصم {price_text} نقطة)*")  
        elif item_type == 'media':  
            await call.message.answer_photo(photo=content, caption=f"📸 {name}\n\n*(تمت إضافة الملف لمشترياتك وتم خصم {price_text} نقطة)*")
...
async def build_step4(message: types.Message, state: FSMContext):
    ...
    if data['type'] in ['folder', 'link']:  
        await state.update_data(price=0.0) # المجلدات والروابط مجانية
        if data['type'] == 'folder':
            cursor.execute('INSERT INTO elements (parent_id, type, name, content, price) VALUES (?, ?, ?, ?, ?)', (data['parent_id'], 'folder', data['name'], 'none', 0.0))  
            conn.commit()  
            await message.answer("✅ تم إنشاء القسم البرمجي المجلد بنجاح تام!")  
            await state.clear()  
        else:
            await message.answer("📥 أرسل الآن الرابط الخارجي (يبدأ بـ http):")
            await state.set_state(SystemStates.wait_content)
    else:  
        # إذا كان منتج (نص أو صورة) نطلب السعر
        await message.answer("💰 أرسل سعر هذا المنتج بالنقاط (مثال: 10 أو 5.5):")  
...
async def build_step_price(message: types.Message, state: FSMContext):
    try:
        ...
        return await message.answer("❌ يرجى إرسال السعر كـ رقم فقط!")
...
async def build_step5(message: types.Message, state: FSMContext):
    ...
    cursor.execute('INSERT INTO elements (parent_id, type, name, content, price) VALUES (?, ?, ?, ?, ?)', (data['parent_id'], data['type'], data['name'], content, price))  
    conn.commit()  
    await message.answer("🎯 تم تثبيت الزر الجديد والسعر بنجاح مذهل ضمن هيكلية المتجر!")  
    await state.clear()
