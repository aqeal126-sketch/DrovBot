...
async def start(message: types.Message):
    ...
    conn.commit()
    await message.answer("أهلاً بك في متجرنا الرقمي:", reply_markup=get_main_kb())

# --- لوحة التحكم (محمية) ---
@dp.callback_query(F.data == "admin_panel")
async def admin_panel(call: types.CallbackQuery):
    if call.from_user.id != ADMIN_ID:
        await call.answer("❌ هذه اللوحة للمطور فقط!", show_alert=True)
        return
    
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📢 إذاعة للمستخدمين", callback_data="broadcast")],
        [InlineKeyboardButton(text="🔙 رجوع", callback_data="back_main")]
    ])
    await call.message.edit_text("⚙️ لوحة تحكم المطور:", reply_markup=kb)

# --- الإذاعة ---
@dp.callback_query(F.data == "broadcast")
async def broadcast_start(call: types.CallbackQuery, state: FSMContext):
    await call.message.answer("أرسل رسالة الإذاعة:")
...
async def broadcast_exec(message: types.Message, state: FSMContext):
    ...
    await message.answer("✅ تم الإرسال للجميع.")
    await state.clear()
